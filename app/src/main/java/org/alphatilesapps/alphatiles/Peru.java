		...
		for (int i = 0; i < TILE_BUTTONS.length; i++)
		{
			...
			else
			{
				...
				case 1:
					...
					tempArray1.set(0, Start.tileList.get(Start.tileList.returnPositionInAlphabet(parsedWordArrayFinal.get(0))).altTiles[incorrectLapNo - 1]); // KRP
				...
				case 2:
					...
					int randomNum3 = rand.nextInt(tileLength - 1);       // KRP // this represents which position in word string will be replaced
					int randomNum4 = rand.nextInt(Start.tileList.size());       // KRP // this represents which game tile will overwrite some part of the correct word

					List<String> tempArray2 = new ArrayList<>(parsedWordArrayFinal);
					tempArray2.set(randomNum3, Start.tileList.get(randomNum4).baseTile); // KRP
...
/*
	protected void setAllTilesUnclickable()
	{
		...
		for (int wordChoice : TILE_BUTTONS)
		{               // RR
