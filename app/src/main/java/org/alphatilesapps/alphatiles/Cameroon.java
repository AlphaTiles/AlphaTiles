...
public class Cameroon extends GameActivity {
    ...
    protected void onCreate(Bundle state) {
        ...
        useImage = challengeLevel / 100 != 2; // slot 1: 1__ --> TRUE = image visible, 2__ --> FALSE = image hidden
        substituteAll = (challengeLevel / 10) % 10 != 2; // slot 2: _1_ --> TRUE = substitute all, _2_ --> FALSE = substitute one
        useDistractors = challengeLevel % 10 == 2; // slot 3: __2 --> TRUE = substitute with distractors, __1 --> FALSE = substitute with random
        ...
        for (int idx = 0; idx < 4; idx++) {
            ...
            int i = idx;
            selector.setOnClickListener((view) -> respondToSelectorSelection(selector, i));
    ...
    private List<List<Start.Tile>> mutateWord(ArrayList<Start.Tile> word) {
        ...
        if (substituteAll) {
            ...
            for(int row = 0; row < 3; row++) {
                int col;
                List<Start.Tile> replacements;
                do {
                    col = rand.nextInt(word.size());
                    replacements = replacementTiles(word.get(col), useDistractors);
                }
    ...
    private void respondToSelectorSelection(View selector, int idx) {
