...
public class UnitedStates extends GameActivity
{
	...
	int neutralFontSize;
	String[] selections = new String[]{"", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", ""}; // KRP
	...
	public void playAgain()
	{
		...
		selections = new String[]{"", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", ""}; // KRP


		int lengthOfLOPWord = Integer.MAX_VALUE;  // KRP // Arbitrarily high number to force into the loop
		...
		while (lengthOfLOPWord > upperTileLimit)
		{
			...
			lengthOfLOPWord = parsedWordArrayFinal.size(); // KRP
		}
		...
		for (int b = 0; b < visibleTiles; b += 2)
		{
			...
			if (c < parsedWordArrayFinal.size())
			{
				...
				randomNum2 = rand.nextInt(Start.ALT_COUNT); // KRP // choosing between 2nd and 4th item of game tiles array
				if (randomNum == 0)
				{
		...
		TextView constructedWord = findViewById(R.id.activeWordTextView); // KRP
		constructedWord.setText(""); // KRP
	...
	public void buildWord(String tileNumber, String tileString)
	{
		...
		StringBuilder displayedWord = new StringBuilder(); // KRP
